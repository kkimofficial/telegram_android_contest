...
public class DrawerLayoutContainer extends FrameLayout {
    ...
    public DrawerLayoutContainer(Context context) {
        ...
        minDrawerMargin = (int) (MIN_DRAWER_MARGIN * DisplayController.density + 0.5f);
    ...
    public void openDrawer(boolean fast) {
        ...
        if (DisplayController.isTablet && parentActionBarLayout != null && parentActionBarLayout.parentActivity != null) {
            DisplayController.hideKeyboard(parentActionBarLayout.parentActivity.getCurrentFocus());
        }
        ...
        if (!parentActionBarLayout.checkTransitionAnimation()) {
            ...
            if (allowOpenDrawer && parentActionBarLayout.fragmentsStack.size() == 1) {
                if (ev != null && (ev.getAction() == MotionEvent.ACTION_DOWN || ev.getAction() == MotionEvent.ACTION_MOVE) && !startedTracking && !maybeStartTracking) {
                    ...
                    if (maybeStartTracking && !startedTracking && (dx > 0 && dx / 3.0f > Math.abs(dy) && Math.abs(dx) >= DisplayController.pixels(0.2f, true) || dx < 0 && Math.abs(dx) >= Math.abs(dy) && Math.abs(dx) >= DisplayController.pixels(0.3f, true))) {
                        ...
                        if (!beginTrackingSent) {
                            if (((Activity)getContext()).getCurrentFocus() != null) {
                                DisplayController.hideKeyboard(((Activity)getContext()).getCurrentFocus());
                            }
    ...
    protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
        ...
        if (scrimOpacity > 0 && drawingContent) {
            ...
            final float alpha = Math.max(0, Math.min(drawerPosition / DisplayController.dp(20), 1.0f));
            if (alpha != 0) {
