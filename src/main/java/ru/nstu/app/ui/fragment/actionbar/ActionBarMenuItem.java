...
public class ActionBarMenuItem extends FixedFrameLayout {
    ...
    private int menuHeight = DisplayController.dp(16);
...
//                    }
//                };
//                Droid.doRunnableUI(showMenuRunnable, 200);
...
//    }
    ...
    public TextView addSubItem(int id, String text, int icon) {
        ...
        textView.setBackgroundResource(R.drawable.st_list_selector);
        if (!LocaleController.isRTL) {
            textView.setGravity(Gravity.CENTER_VERTICAL);
        } else {
            textView.setGravity(Gravity.CENTER_VERTICAL | Gravity.RIGHT);
        }
        textView.setPadding(DisplayController.dp(16), 0, DisplayController.dp(16), 0);
        textView.setTextSize(DisplayController.sp(18));
        textView.setMinWidth(DisplayController.dp(196));
        textView.setTag(id);
        textView.setText(text);
        if (icon != 0) {
            textView.setCompoundDrawablePadding(DisplayController.dp(12));
            if (!LocaleController.isRTL) {
    ...
    public void toggleSubMenu() {
        ...
        if (showMenuRunnable != null) {
            Droid.cancelRunnableUI(showMenuRunnable);
        ...
        if (popupWindow == null) {
            ...
            popupLayout.measure(MeasureSpec.makeMeasureSpec(DisplayController.dp(1000), MeasureSpec.AT_MOST), MeasureSpec.makeMeasureSpec(DisplayController.dp(1000), MeasureSpec.AT_MOST));
        ...
        if (popupLayout.getMeasuredWidth() == 0) {
            if (subMenuOpenSide == 0) {
                ...
                popupWindow.showAsDropDown(this, -DisplayController.dp(8), getOffsetY());
                popupWindow.update(this, -DisplayController.dp(8), getOffsetY(), -1, -1);
            }
        } else {
            if (subMenuOpenSide == 0) {
                if (showFromBottom) {
                    ...
                    if(Droid.activity.getMessagesCreator().isShowindPhotoViewPopupWindow()) {
                        popupWindow.showAtLocation(Droid.activity.getWindow().getDecorView(), Gravity.TOP | Gravity.RIGHT, 0, DisplayController.statusBarHeight);
                    } else {
                        popupWindow.showAsDropDown(this, parentMenu.parentActionBar.getMeasuredWidth() - popupLayout.getMeasuredWidth() - getLeft() - parentMenu.getLeft(), getOffsetY());
                    }
                }
    ...
    private int getOffsetY() {
        if (showFromBottom) {
            getLocationOnScreen(location);
            int diff = location[1] - DisplayController.statusBarHeight + getMeasuredHeight() - menuHeight;
            int y = -menuHeight;
            if (diff < 0) {
                y -= diff;
            }
            return y - (needOffset ? DisplayController.statusBarHeight : 0);
        } else {
            return -getMeasuredHeight() - (needOffset ? DisplayController.statusBarHeight : 0);
        }
    }
    ...
    public boolean toggleSearch() {
        ...
        if (searchContainer.getVisibility() == VISIBLE) {
            if (listener == null || listener != null && listener.onSearchCollapse()) {
                ...
                DisplayController.hideKeyboard(searchField);
            }
            return false;
        } else {
            ...
            DisplayController.showKeyboard(searchField);
            if (listener != null) {
    ...
    public ActionBarMenuItem setIsSearchField(boolean value) {
        if (value && searchContainer == null) {
            ...
            layoutParams.leftMargin = DisplayController.dp(6);
            ...
            layoutParams2.height = DisplayController.dp(36);
            layoutParams2.rightMargin = DisplayController.dp(48);
            ...
            layoutParams2.width = DisplayController.dp(48);
