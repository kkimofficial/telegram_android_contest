...
public class Emoji {
    ...
    static {
        int emojiFullSize;
        if (DisplayController.density <= 1.0f) {
            emojiFullSize = 30;
        } else if (DisplayController.density <= 1.5f) {
            emojiFullSize = 45;
        } else if (DisplayController.density <= 2.0f) {
            emojiFullSize = 60;
        } else {
            emojiFullSize = 90;
        }
        drawImgSize = DisplayController.dp(20);
        if (DisplayController.isTablet) {
            bigImgSize = DisplayController.dp(40);
        } else {
            bigImgSize = DisplayController.dp(30);
        }
    ...
    private static void loadEmoji(final int page) {
        try {
            ...
            File imageFile = Droid.activity.getFileStreamPath(imageName);
            if (!imageFile.exists()) {
                InputStream is = Droid.activity.getAssets().open("emoji/" + imageName);
                Droid.copyFile(is, imageFile);
                is.close();
            }
            ...
            NativeProvider.loadBitmap(imageFile.getAbsolutePath(), bitmap, imageResize, width, height, stride);

            imageName = String.format(Locale.US, "emoji%.01fx_a_%d.jpg", scale, page);
            imageFile = Droid.activity.getFileStreamPath(imageName);
            if (!imageFile.exists()) {
                InputStream is = Droid.activity.getAssets().open("emoji/" + imageName);
                Droid.copyFile(is, imageFile);
                is.close();
            ...
            Droid.doRunnableUI(new Runnable() {
                ...
                public void run() {
                    ...
                    if(dialogsCreator != null) {
                        dialogsCreator.notify(Notification.EMOJI);
                    }
                    MessagesCreator messagesCreator = Droid.activity.getMessagesCreator();
                    if(messagesCreator != null) {
                        messagesCreator.notify(Notification.EMOJI);
                    }
                }
            });
        } catch(Throwable x) {
    ...
    private static void xorBitmaps(Bitmap bitmap1, Bitmap bitmap2) {
...
//                bitmap1.setPixel(i, j, Color.argb((pixel2 / -16777216) - 1, Color.red(pixel1), Color.green(pixel1), Color.blue(pixel1)));
                if(Math.abs(pixel2) > 0xE9E9E9) {
                    bitmap1.setPixel(i, j, 0x00000000);
                }
    ...
    public static class EmojiSpan extends ImageSpan {
        ...
        private int size = DisplayController.dp(20);
        ...
        public int getSize(Paint paint, CharSequence text, int start, int end, Paint.FontMetricsInt fm) {
            ...
            if (fontMetrics == null) {
                ...
                int offset = DisplayController.dp(8);
                int w = DisplayController.dp(10);
                fm.top = -w - offset;
                fm.bottom = w - offset;
                fm.ascent = -w - offset;
                fm.leading = 0;
